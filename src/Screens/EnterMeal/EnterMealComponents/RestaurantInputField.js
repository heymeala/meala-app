    ...
    <View>
      {props.gpsEnabled ? null : (
        <TouchableOpacity onPress={() => Linking.openSettings()}>
          <Text style={{ color: 'red', padding: 8 }}>{t('AddMeal.gpsPermission')}</Text>
        </TouchableOpacity>
      )}
      <Input
      ...
      <View style={{ paddingBottom: 8 }}>
        {toggleFocus &&
          placesFiltered.map((items, index) => (
            <View key={index}>
              <TouchableOpacity
                onPress={() => props.handleRestaurantPress(items.restaurant_name, items.id, 'Own')}>
        ...
        {toggleFocus &&
          googlePlacesFiltered.map(items => (
            <View key={items.place_id}>
              <TouchableOpacity
                onPress={() => props.handleRestaurantPress(items.name, items.place_id, 'GOOGLE')}>
                <View style={{ flexDirection: 'row', padding: 5 }}>
                  <Icon style={{ paddingTop: 3 }} color="#F9DE1C" name="logo-google" size={20} />
                  <Text style={{ padding: 5 }}>{items.name} </Text>
                  <View style={{ flexDirection: 'column' }}>
                    <Text style={{ fontSize: 10, padding: 5 }}>{items.formatted_address}</Text>
                  </View>
                </View>
              </TouchableOpacity>
              <Divider />
