...
const NightscoutSettingsScreen = props => {
  ...
  function validate() {
    if (nightscoutUrl) {
      const checkUrl = nightscoutUrl.slice(-1) === '/' ? nightscoutUrl.slice(0, -1) : nightscoutUrl;
    ...
    <>
      <ScrollView style={styles.container}>
        <View>
          ...
          />
          <Text style={{ padding: 20, paddingBottom: 10 }}>{t('Settings.token')}</Text>
          <Input
            containerStyle={{ paddingTop: 25 }}
          ...
          <ListItem.Accordion
            ...
            <CheckBox
              ...
              onPress={() => setNightscoutTreatmentsUpload(!nightscoutTreatmentsUpload)}
            />
            <Text style={{ padding: 20, paddingBottom: 10 }}>{t('Settings.uploadTreatmentsInfo')}</Text>
          </ListItem.Accordion>
        ...
        {Platform.OS === 'ios' ? (
          <View style={{ padding: 20 }}>
            <Text style={{ paddingBottom: 5 }}>{t('Settings.nightscoutRecommendation')}</Text>
          </View>
        ) : null}
      </ScrollView>
      <FAB
